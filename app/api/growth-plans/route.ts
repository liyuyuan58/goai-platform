...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!plan?.title || !plan?.targetMarket) {
      return NextResponse.json({ error: "Invalid growth plan" }, { status: 400 });
    }
    ...
    const rows = await supabaseServerRequest<any[]>("growth_plans", {
      ...
      body: JSON.stringify({
        ...
        company_name: context.companyName || "Unknown company",
        current_country: context.currentCountry || "Unknown",
        target_country: context.targetCountry || plan.targetMarket,
        title: plan.title,
